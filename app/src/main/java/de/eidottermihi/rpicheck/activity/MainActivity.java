...
public class MainActivity extends InjectionActionBarActivity implements ActionBar.OnNavigationListener,
	...
	private void updateQueryDataInView() {
		QueryBean lastQueryData = currentDevice.getLastQueryData();
		final List<String> errorMessages = lastQueryData.getErrorMessages();
		final String tempScale = sharedPrefs.getString(SettingsActivity.KEY_PREF_TEMPERATURE_SCALE,
															  getString(R.string.pref_temperature_scala_default));
		VcgencmdBean vcgencmdInfo = lastQueryData.getVcgencmdInfo();
		coreTempText.setText(helper.formatTemperature(vcgencmdInfo.getCpuTemperature(), tempScale));
		final String freqScale = sharedPrefs.getString(SettingsActivity.KEY_PREF_FREQUENCY_UNIT,
															  getString(R.string.pref_frequency_unit_default));
		armFreqText.setText(helper.formatFrequency(vcgencmdInfo.getArmFrequency(), freqScale));
		coreFreqText.setText(helper.formatFrequency(vcgencmdInfo.getCoreFrequency(), freqScale));
		coreVoltText.setText(helper.formatDecimal(vcgencmdInfo.getCoreVolts()));
		firmwareText.setText(vcgencmdInfo.getVersion());
		lastUpdateText.setText(SimpleDateFormat.getDateTimeInstance().format(lastQueryData.getLastUpdate()));
		// uptime and average load may contain errors
		if (lastQueryData.getAvgLoad() != null) {
			averageLoadText.setText(lastQueryData.getAvgLoad().toString());
		}
		if (lastQueryData.getStartup() != null) {
			uptimeText.setText(lastQueryData.getStartup());
		}
		if (lastQueryData.getFreeMem() != null) {
			freeMemoryText.setText(lastQueryData.getFreeMem().humanReadableByteCount(false));
		}
		if (lastQueryData.getTotalMem() != null) {
			totalMemoryText.setText(lastQueryData.getTotalMem().humanReadableByteCount(false));
		}
		serialNoText.setText(lastQueryData.getSerialNo());
		distriText.setText(lastQueryData.getDistri());
		// update tables
		updateNetworkTable();
		updateDiskTable();
		updateProcessTable();
	...
	private void updateNetworkTable() {
		...
		if (currentDevice != null && currentDevice.getLastQueryData() != null
			...
			for (NetworkInterfaceInformation interfaceInformation : currentDevice.getLastQueryData().getNetworkInfo()) {
	...
	private View createNetworkRow(NetworkInterfaceInformation interfaceInformation) {
		...
		if (interfaceInformation.getWlanInfo() != null) {
			...
			tempRow.addView(createTextView(helper.formatPercentage(wlan.getSignalLevel()), 3));
			tempRow.addView(createTextView(helper.formatPercentage(wlan.getLinkQuality()), 3));
		} else {
	...
	private void updateProcessTable() {
		...
		if (currentDevice != null && currentDevice.getLastQueryData() != null
			...
			for (ProcessBean processBean : currentDevice.getLastQueryData().getProcesses()) {
	...
	private void initSpinner() {
		new AsyncTask<Void, Void, Void>() {
			...
			protected void onPostExecute(Void r) {
				...
				if (deviceCursor.getCount() > 0) {
					// make adapter
					spinadapter = new SimpleCursorAdapter(MainActivity.this, R.layout.support_simple_spinner_dropdown_item, deviceCursor,
																 new String[]{"name", "_id"},
																 new int[]{android.R.id.text1});
					//spinadapter.setDropDownViewResource(R.layout.sherlock_spinner_dropdown_item);
	...
	private void updateResultsInUi() {
		...
		if (currentDevice.getLastQueryData().getException() == null) {
			...
			this.handleQueryException(currentDevice.getLastQueryData().getException());
		}
	}
	...
	private void doRebootOrHalt(String type) {
		...
		if (networkInfo != null && networkInfo.isConnected()) {
			...
			if (currentDevice.getAuthMethod().equals(NewRaspiAuthActivity.SPINNER_AUTH_METHODS[0])) {
				...
				new SSHShutdownTask().execute(host, user, pass, port, sudoPass, type, null, null);
				...
				if (keyfilePath != null) {
					...
					if (privateKey.exists()) {
						new SSHShutdownTask().execute(host, user, null, port, sudoPass, type, keyfilePath, null);
					} else {
	...
	private void doQuery(boolean initByPullToRefresh) {
		...
		if (networkInfo != null && networkInfo.isConnected()) {
			...
			final Boolean hideRoot = Boolean.valueOf(sharedPrefs.getBoolean(SettingsActivity.KEY_PREF_QUERY_HIDE_ROOT_PROCESSES, true));
			...
			if (authMethod.equals(NewRaspiAuthActivity.SPINNER_AUTH_METHODS[0])) {
				...
				final String keyFileContent = currentDevice.getKeyFileContent();
				if (keyFileContent != null) {
					final File privateKey = new File(keyFileContent);
					if (privateKey.exists()) {
						keyContent = keyFileContent;
						canConnect = true;
					} else {
						Toast.makeText(this, "Cannot find keyfile at location: " + keyFileContent, Toast.LENGTH_LONG).show();
					}
				} else {
	...
	private class SSHQueryTask extends AsyncTask<String, Integer, QueryBean> {
		...
		protected QueryBean doInBackground(String... params) {
			...
			try {
				...
				publishProgress(72);
				List<ProcessBean> processes = raspiQuery.queryProcesses(!hideRootProcesses);
				publishProgress(80);
				final List<NetworkInterfaceInformation> networkInformation = raspiQuery.queryNetworkInformation();
				publishProgress(90);
